...
def _process_flag(full_key_name: str, abbr_key_name: Optional[str] = None,
              has_value_part: Optional[bool] = False, **kwargs) -> List[str]:
    """
...
def _get_tag_flag(**kwargs) -> List[str]:
    ...
    """
    return _process_flag(full_key_name="tag", abbr_key_name="t", 
                         has_value_part=True, **kwargs)
...
def _process_command_flags(kit_cmd_name: str, **kwargs) -> List[str]:
    ...
    """
    flags = []
    if kit_cmd_name in ["info", "inspect", "list", "login", "pull", "push",
                        "remove", "unpack"]:
